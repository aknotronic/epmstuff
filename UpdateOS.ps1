...
.MODIFIED BY: Matias Magnus Andersen
...
<#
.SYNOPSIS
Installs the latest Windows Quality, Feature updates and Drivers if the device is in Pre-Provision mode.
.DESCRIPTION
...
# Start logging
Start-Transcript "$($env:ProgramData)\Microsoft\IntuneManagementExtension\Logs\UpdateOS.log" -Force -Append

$DomainName = "<InsertYourDomainNameHere>"

# Enumerates keys to see if it can find "URL=<fooUser@domain.com>" and if so, run Windows Update.
$ChildRegistries = (Get-ChildItem -Path "HKLM:\Software\Microsoft\Enrollments").Name -Replace ("HKEY_LOCAL_MACHINE","HKLM:")
foreach($ChildRegistryName in $ChildRegistries)
{
    $Registry = Get-ItemProperty -Path $ChildRegistryName
    if($Registry.UPN -eq "fooUser@$DomainName")
    {
        Write-Host "URL=fooUser@$DomainName found in $ChildRegistryName. Pre-Provision mode detected. Will run Windows Update!" -ForegroundColor Green      
        # Main logic
        $needReboot = $false
        ...
        {
        Write-Host "URL=fooUser@$DomainName not found in $ChildRegistryName. Pre-Provision mode not detected." -ForegroundColor Red
        }    
    }
